      ...
      class ExternalEventsController < Admin::ApplicationController
        ...
        def create
          ...
          CreateExternalEvent.call(@form) do
            on(:ok) do
              flash[:notice] = I18n.t("create.success", scope: "decidim.event_calendar.admin.external_events")
              redirect_to external_events_path
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("create.invalid", scope: "decidim.event_calendar.admin.external_events")
              render action: "new"
            end
          end
        end
        ...
        def update
          ...
          UpdateExternalEvent.call(current_external_event, @form) do
            on(:ok) do
              flash[:notice] = I18n.t("update.success", scope: "decidim.event_calendar.admin.external_events")
              redirect_to external_events_path
            end

            on(:invalid) do
              flash.now[:alert] = I18n.t("update.invalid", scope: "decidim.event_calendar.admin.external_events")
              render :edit
            end
          end
        end
        ...
        def destroy
          ...
          DestroyExternalEvent.call(current_external_event) do
            on(:ok) do
              flash[:notice] = I18n.t("destroy.success", scope: "decidim.event_calendar.admin.external_events")
              redirect_to external_events_path
            end
          end
        end
        ...
        def external_events
          @external_events ||= EventCalendar::ExternalEvent.where(organization: current_organization).page(params[:page]).per(15)
        end
